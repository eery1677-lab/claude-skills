...
### A third real bug: the ingestion scraper's tag whitelist silently drops table-based content

Found 2026-09-15/16 in the same 나만의콘텐츠생성기 build, while the user asked "does the wiki actually have this product's exact frame size?" — checked directly and it didn't, even though the product's real spec sheet (frame/lens/temple mm measurements, material) was sitting right there on the brand's own website. The ingestion path (`_fetch_url_as_plain_text`) uses a real headless-browser render (Playwright) then extracts text with a tag whitelist: `find_all(['p', 'h1', 'h2', 'h3', 'h4', 'li'])`. That whitelist has no `table`/`tr`/`td` — so any content laid out as an HTML `<table>` (which is *most* e-commerce product spec sheets: size charts, material tables, price comparisons) is silently invisible to the extractor, with zero error anywhere. Re-scraping 8 already-registered brand URLs with the fix applied recovered real content on 4 of them (+1268, +1263, +421, +254 characters) — the other 4 genuinely had no table content, so the bug's actual damage rate on real data was ~50% of registered sources, not a rare edge case.

**The fix**: don't just add more tags to the whitelist (a `<table>`'s useful structure — which cell belongs to which row/column header — is lost if you flatten it through the same per-element `get_text()` loop as paragraphs). Extract tables separately: walk each `<table>`, join each row's cells with a delimiter (`" | "`) into one line, and `decompose()` the table out of the tree afterward so the same cell text doesn't also get picked up a second time by the p/h/li scan.

**The general lesson**: a "grab text from these semantic tags" ingestion filter is really a *positive* whitelist of everywhere the pipeline is willing to look — and the moment the filter was written, whoever wrote it was almost certainly picturing prose (articles, docs), not tabular data. Any ingestion feature aimed at real-world commercial/product pages needs to explicitly ask "where do *this class of site* actually put their most valuable numbers" — for retail/e-commerce that's overwhelmingly `<table>`, for financial or comparison content it's the same — rather than assuming the whitelist that worked for blog-post scraping generalizes. A `len(extracted) `spot check against what a human sees in the browser for 2-3 real target pages, done once when the scraper is first built, would have caught this immediately; it went unnoticed for weeks because the failure produces no error, just a shorter-than-expected note that still looks plausible.
