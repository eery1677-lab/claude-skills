...
## Known limitation: this reduces hallucination, it does not eliminate it

Caught live in the same project, after the pattern had already been shipped and was being used normally: a staged analysis reported "좋아요 1013개" (1,013 likes) in its trend section — a number that does not appear anywhere in the ground-truth data (the real figure, for a different post, was 13 likes; the model appears to have fused two real numbers from nearby lines into one fabricated one). The server logs confirmed the verify endpoint *was* called and returned 200 — the fact-check pass ran, and still missed it.

The reason: the verifier is doing the same kind of pattern-completion a small local model always does, just with a narrower job. Give it a paragraph with several real numbers packed close together and it can still fuse or skim past one, especially when the fabricated value is plausible-looking (fusing "13" and "10" into "1013" reads as a typo, not a lie, on a casual pass). This isn't a bug in the implementation above — it's the actual, current ceiling of a single low-temperature verification pass on a local model.

**What this means in practice**: keep treating this pattern as risk-reduction, not a correctness guarantee. Don't remove a human-in-the-loop review step because verification is in place, and don't advertise verified output to an end user as "fact-checked" without a qualifier. If a specific number matters a lot (e.g. it drives a business decision), a second independent verify pass, or a plain code-level regex check that the number literally appears in the ground-truth string, catches this class of error far more reliably than another LLM pass — reach for that when the stakes justify it.

## Variant: self-critique-via-schema + one corrective retry (for structuring/synthesis tasks)

The pattern above uses a *separate* low-temperature verify call. There's a sibling pattern for tasks where the model is condensing/restructuring a source document into a note or summary (not citing numbers from a data block) — here it's cheaper and works well to have the *same* call self-critique via extra JSON-schema fields, then do one corrective retry if it fails, instead of a second call:

1. The structuring prompt's JSON schema includes `grounded: boolean` and `issues: string[]` fields alongside the actual output fields, with instructions to compare its own output against the source and honestly flag anything it isn't sure is actually in the source.
2. If `grounded` comes back `false`, **don't just block and stop** — that wastes the model's own diagnosis. Feed the `issues` array and the previous attempt's output straight back into a second call of the *same* prompt, with an added instruction block: "your previous attempt failed self-verification for these specific reasons — fix only those parts using content that's actually in the source, or remove the unsupported claim entirely rather than keep guessing."
3. Cap it at **one** retry. If the second attempt is still `grounded: false`, stop there and surface the block to the user (or skip the save) — don't loop. This is the same "don't over-trust automated correction" discipline as the length-safety-net above: a retry loop hides the failure instead of surfacing it, and there's no evidence a third attempt does better than a second.
4. This is strictly better than a blind "just click retry again" because a fresh, feedback-free retry re-rolls the same dice — it has no memory of what specifically went wrong last time and can easily repeat the identical mistake. Pointing at the exact flagged issue turns "regenerate and hope" into "fix this specific thing."

Shipped example: a "save this result to the knowledge wiki" feature ([[second-brain-wiki-rag]]) asks the structuring call to self-report `grounded`/`issues` on whether its condensed note invented anything not in the raw source. On `grounded: false`, the frontend calls the same `/structure/prep` endpoint again, this time passing `previous_attempt` (the rejected note) and `previous_issues` (the flagged problems) as extra fields; the backend appends a "이전 시도가 실패했습니다, 이 부분만 원본 근거로 고쳐라" block to the system prompt for that one retry only.
